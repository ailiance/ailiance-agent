...
/**
 * Converts Dirac storage messages to Anthropic API format with optional cache control.
 * Adds ephemeral cache control to the last two user messages to prevent them from being
 * stored in Anthropic's cache.
 *
 * @param diracMessages - Array of Dirac storage messages to convert
...
export function sanitizeAnthropicMessages(
	diracMessages: Array<DiracStorageMessage | Anthropic.MessageParam>,
	...
	const userMsgIndices = diracMessages.reduce((acc, msg, index) => {
		if (msg.role === "user") {
			acc.push(index)
		}
		return acc
