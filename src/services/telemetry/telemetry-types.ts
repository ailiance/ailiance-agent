/**
 * Shared telemetry type, enum and interface definitions.
 *
 * Extracted verbatim from TelemetryService for readability. `TelemetryService`
 * re-exports every symbol declared here so existing imports
 * (`from "./TelemetryService"` and the package `index.ts`) keep working
 * unchanged.
 */
