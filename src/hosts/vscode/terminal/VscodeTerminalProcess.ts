		...
		const returnCurrentTerminalContents = async () => {
			try {
				...
				if (terminalSnapshot && terminalSnapshot.trim()) {
					const fallbackMessage = `No output. Here's the current terminal's content to help you get the command's output:\n\n${terminalSnapshot}`
		...
		if (terminal.shellIntegration && terminal.shellIntegration.executeCommand) {
			...
			for await (let data of stream) {
				...
				if (latestCompletionMatch) {
					...
					// If we found a completion marker, we should process the data up to that marker and then break
					// This prevents hanging if the stream doesn't close naturally
					const markerIndex = latestCompletionMatch.index
					if (markerIndex !== undefined) {
						data = data.slice(0, markerIndex)
					}
					didComplete = true
				}
