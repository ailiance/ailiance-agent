...
`DiracAgent` = implémentation `acp.Agent` découplée de stdio : enveloppe un `Controller`
core par session et traduit ses `DiracMessage` en `acp.SessionUpdate` via EventEmitter.

## Files

| Fichier | Rôle |
|---------|------|
| `DiracAgent.ts` | Classe `DiracAgent` (L88) implémente `acp.Agent`. `newSession` L273 (1 `Controller`/session), `loadSession` L324 (rehydrate depuis history, lazy), `prompt` L737 (entrée principale), `cancel` L1203, `setSessionMode` L1238, `unstable_setSessionConfigOption` L599 (mode/provider/model/reasoning/thinking). Émission : `emitSessionUpdate` L1285, `sendAvailableCommands` L1306, `publishSessionSetupUpdates` L374 |
| `messageTranslator.ts` | `translateMessage` L176 (DiracMessage → updates). `translateSayMessage` L209, `translateAskMessage` L450, `translateToolMessage` L673, `translateCommandMessage` L818, `translateCommandOutputMessage` L862. `parseWebSearchMarkerText` L73, `TOOL_KIND_MAP` L19, `parseUnifiedDiff` L104 |
| `permissionHandler.ts` | `handlePermissionResponse` L110 (outcome ACP → `DiracAskResponse`). `requiresPermission` L79, `ASK_TYPE_PERMISSION_MAP` L39, `AutoApprovalTracker` L178 (non câblé dans DiracAgent) |
| `DiracSessionEmitter.ts` | `DiracSessionEmitter` L34 — wrapper typé sur `EventEmitter` (maxListeners 20), clés = `SessionUpdateType` + `error` |
...
| `public-types.ts` | Types lib-safe : `DiracAgentOptions`, `PermissionHandler` L45, `DiracSessionEvents` L58, `AcpSessionState`, `AcpSessionStatus` L131 |
...
- Chaque `DiracMessageChange` → `processMessageWithDelta` (L1090) : texte/reasoning streamés en
...
- Émission : `emitterForSession(sessionId)` → `DiracSessionEmitter`. `AcpAgent` (cli/src/acp/)
...
- **1 Controller par session**, pas via `CliWebviewProvider`. Ce dernier sert le mode TUI ;
  `DiracAgent` instancie le `Controller` core directement (chemin ACP/lib, pas le chemin Ink).
...
- **`AutoApprovalTracker`** (permissionHandler.ts L178) existe mais n'est PAS branché par
  DiracAgent — l'auto-approval réel vient des options `allow_always` côté client + state core.
